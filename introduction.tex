...
In one task-slot may run multiple subtasks as well, if this way performance can be increased. Just to mention a trivial example, executing two subsequent map transformation on the same stream partition in different slots would lead to totally  needless communication overhead.With \textit{chaining} tasks slots can hold enire pipelines of the job. 
