...
t\subsubsection{Linear regression} \label{linearregression}
...
A possible implementation of executing asynchronous barrier snapshotting with guarantees for iterative datastream jobs can be achieved by adding the following algorithm to the basic ABS:
\begin{itemize}
...
\citep{pull_1668}(https://github.com/apache/flink/pull/1668)
...
\subsubsection{The Flink program}
...
\begin{equation}
 y=2x+0
\end{equation}
I marked the parameters wanted $\Theta_0$ (theta\_0) and $\Theta_1$ (theta\_1), where $y = \Theta_1*x+ \Theta_1$.
...
A Flink program like the one described above is executed in \textit{Flink runtime}, as a client program. It describes and send the dataflow to the runtime engine as a \textit{job}, than it either can disconnect, or can stay connected ot receive progress reports.

The Flink runtime(Figure \ref{fig:flinkrt}) which executes the computations specified in client consists of two types of processes:
\begin{description}
...
In the runtime always must be at least one master process, but in high availibility setups there are other masters as well that are stanby while the \textit{leader} does its job, but one of them takes leaders place if that becomes unavailible form same reason.
\item[Worker processes] or \textit{Task managers} execute the tasks or subtasks of the dataflow. They are responsible to buffer data, and manage data exchange along streams. Naturally at least one worker process is always needed.
\end{description}
\cite{flink concepts} (https://ci.apache.org/projects/flink/flink-docs-master/concepts/concepts.html) 

\begin{figure}[!ht]
  \centering    
      \includegraphics[width=1\textwidth]{figures/flinkrt.svg}
  \caption{Execution of Flink program\cite{flink concepts}}
  \label{fig:flinkrt}
\end{figure}  

Workers as well as masters of the runtime are JVM processes. A worker may execute one or more subtasks paralelly in separate threads in so-called \textit{task-slots}, that represent a fixed subset Task manager's resources. Each subtasks have its own managed memory, though currently they compete for CPU time.

In one task-slot may run multiple subtasks as well, if this way performance can be increased. Just to mention a trivial example, executing two subsequent map transformation on the same stream partition in different slots would lead to totally  needless communication overhead.

With \textit{chaining} tasks slots can hold enire pipelines of the job. 



\subsection{Performance measurement}
The above described Flink program what implements the linear regression algorithm has been tested regarding the following questions:
\begin{itemize}
\item How ABS checkpointing method infuences the execution time of a flink program?
...
\item And at what extent can cyclic ABS to help the execution of iterative computations?
\end{itemize}
To test the different approaches the program has been run several times in two version of the Flink runtime, first in one with the official ABS checkpointing method and second  in an experimental version which uses the iterative fault tolerance technique too along with ABS. (It may worth to mention that the official version of the runtime (Flink 1.1)still  does not allow users to use checkpointing in iterative topologies, but it is possible to force it to do that.)
 
\subsubsection{Runtime overhead of checkpointing}
To examine the increase of execution time of Flink jobs the execution environment was configured with a series of different checkpointing intervals: The used snapshotting frequencies are in the range of 0.5sec - 3.5 sec. The base line of the experiment is given by the average of the execution times of running the linear regression algorithm without any checkpointing method. Figure \ref{fig:checkpoint} summarizes the result of the experiment: 

\begin{figure}[!ht]
  \centering    
      \includegraphics[width=0.7\textwidth]{figures/checkpointing_overhead.png}
  \caption{Execution time overhead with checkpointing}
  \label{fig:checkpoint}
\end{figure}     

The results of the experiments demonstrate clearly, taking a snapshot using  ABS technique with added support for cyclic topologies requires slightly more execution time compared to simple ABS, but in the experimental program the difference is not really significant. If we compare the time of convergence in case of using any back-up technique to pure execution, the measurements proves that due to asynchronous and lightweight nature of ABS the execution time barely changes. The average termination time of the particular algorithm was in both cases around $6s$, 
\subsubsection{Failure recovery}
Most important characteristic of state backup and recovery mechanism naturally describes, how systems behave when a failure happened. In the second  the experiments examined this behavior of of Flink runtime in presence of the different ABS variants. In the experiment artficial errors have been thrown in different time intervals. The first part of Figure \ref{fig:failures} shows the cases, where no error has happened, the only difference to baseline here is the overhead of snapshotting.(In these cases the algorithm reaches convergence before the error is triggered.) 

\begin{figure}[!ht]
  \centering    
      \includegraphics[width=0.7\textwidth]{figures/failures_impact.png}
  \caption{Impact of failures on convergence time}
  \label{fig:failures}
\end{figure}     

When a failure happens original ABS restores the state of the checkointed operators, but the records that were in transition on communication channels will be lost. These records then will be resent by the source of execution graph, not respecting whether they are processed by some task or not. This leads to violating of principle of exactly-once-processing.

Usually exactly once processing guarantees are not required when we want to execute algorithms, whose purpose is to reach convergence. Loosing some records can slow down the termination of these algorithms, but other cases that can even speed it up. 

Nevertheless the test cases have proven that in some cases as in this particular task, it is possible  to bias the computation to the extent that recovering takes more time than elapsed since the beginning of computations. In cases of repeating failures it is possible as well that  This can be observed in Figure \ref{fig:failures} on the right side, where the processes terminated with timeout. Timeout in our case was specified for 90 seconds.  

On the other hand adding logging of records which are in transition along the streams ABS can highly speed up the termination even in case of continuous failures, with relatively small additional latency. 
